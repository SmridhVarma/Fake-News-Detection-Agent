...
OUTPUT_DIR = "./evaluation_outputs/v1"
...
metrics_csv_path = os.path.join(OUTPUT_DIR, "basic_v1_metrics_summary.csv")
metrics_df.to_csv(metrics_csv_path, index=False)

cm_path = os.path.join(OUTPUT_DIR, "basic_v1_confusion_matrix.png")
plot_confusion_matrix(
    cm=cm,
    labels=["FAKE (0)", "REAL (1)"],
    title=f"Confusion Matrix — {selected_model_name} (v1)",
    save_path=cm_path,
)

summary_txt_path = os.path.join(OUTPUT_DIR, "basic_v1_summary.txt")
with open(summary_txt_path, "w", encoding="utf-8") as f:
    f.write("=== BASIC V1 SUMMARY ===\n\n")
